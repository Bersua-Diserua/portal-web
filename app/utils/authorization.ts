...
export async function setAuthorization(auth: Authorization, headers: Headers = new Headers()) {
...
export async function getAuthorization(request: Request) {
  ...
  if (!auth || typeof auth !== "object" || !("accessToken" in auth)) return null
...
declare global {
  type Auth = Awaited<ReturnType<typeof getRequiredAuth>>
}
